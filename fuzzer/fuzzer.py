...
# Ensure python can find the target_agent module from the root directory
sys.path.append(os.path.abspath('.'))
from target_agent.vulnerable_app import query_vulnerable_agent
...
def run_fuzzer():
    ...
    print("Target: Vulnerable HR Agent (Llama 3.1-8B-Instant)\n")

    for vector_name, payload in PAYLOADS.items():
        print(Style.BRIGHT + f"Executing {vector_name}...")
        print(f"Payload: {payload}")
        
        response = query_vulnerable_agent(payload)
